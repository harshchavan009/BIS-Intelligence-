...
export const SimulatedBadge: React.FC<SimulatedBadgeProps> = ({ className = '', defaultExpanded = false }) => {
  ...
  return (
    <div className={`rounded-md border border-slate-300 bg-slate-50/95 text-slate-800 text-xs transition-all ${className}`}>
      <div 
        onClick={() => setExpanded(!expanded)}
        className="flex items-center justify-between px-3 py-1.5 cursor-pointer select-none hover:bg-slate-100/70 transition-colors rounded-md"
        role="button"
        aria-expanded={expanded}
        aria-label="Toggle demo mode simulated lookup explanation"
      >
        <div className="flex items-center gap-1.5 font-mono text-[11px] font-bold text-slate-700 uppercase tracking-wider">
          <Info className="w-3.5 h-3.5 text-slate-500 flex-shrink-0" />
          <span>Demo Mode — Simulated Lookup</span>
        </div>
        <div className="flex items-center gap-1 text-[10.5px] text-slate-600 font-sans">
      ...
      {expanded && (
        <div className="px-3 pb-2.5 pt-1 text-[11px] leading-relaxed border-t border-slate-200 text-slate-700 font-sans space-y-1">
          <p>
