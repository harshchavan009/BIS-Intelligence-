...
export const ScreenReaderModal: React.FC = () => {
  ...
  return (
    ...
    >
      <div className="bg-surface max-w-2xl w-full rounded-lg shadow-2xl border border-border overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        {/* Header */}
        ...
        {/* Body */}
        <div className="p-6 space-y-4 max-h-[75vh] overflow-y-auto text-text-primary text-sm leading-relaxed">
          <p className="text-text-secondary font-medium">
            {t('a11y.screen_reader_desc')}
          </p>

          <div className="bg-surface-alt border border-border rounded p-4 space-y-3">
            <h3 className="font-semibold text-xs uppercase tracking-wider text-brand-accent flex items-center gap-1.5">
              <CheckCircle2 className="w-4 h-4 text-status-success" />
              {language === 'hi' ? 'समर्थित स्क्रीन रीडर्स एवं सेटअप गाइड' : 'Supported Screen Readers & Setup Guidance'}
            </h3>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 text-xs">
              <div className="p-3 bg-surface rounded border border-border flex flex-col justify-between space-y-2">
                <div>
                  <span className="font-bold text-text-primary block">NVDA (Windows)</span>
                  <span className="text-text-secondary text-[11px]">Free & Open Source</span>
                </div>
                <a
                  href="https://www.nvaccess.org/download/"
                  target="_blank"
                  rel="noopener noreferrer"
                  className="inline-flex items-center gap-1 text-[11px] text-brand-primary hover:text-brand-accent font-medium underline"
              ...
              <div className="p-3 bg-surface rounded border border-border flex flex-col justify-between space-y-2">
                <div>
                  <span className="font-bold text-text-primary block">JAWS (Windows)</span>
                  <span className="text-text-secondary text-[11px]">Commercial Standard</span>
                </div>
                <a
              ...
              <div className="p-3 bg-surface rounded border border-border flex flex-col justify-between space-y-2">
                <div>
                  <span className="font-bold text-text-primary block">VoiceOver</span>
                  <span className="text-text-secondary text-[11px]">macOS & iOS Built-in</span>
                </div>
        ...
        {/* Footer */}
        <div className="bg-surface-alt px-6 py-3 border-t border-border flex justify-end">
          <button
            onClick={() => setScreenReaderModalOpen(false)}
            className="px-4 py-1.5 bg-brand-primary text-white text-xs font-medium rounded hover:brightness-110 transition-colors"
          >
