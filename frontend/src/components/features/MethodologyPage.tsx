  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 space-y-8 font-sans">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="p-5 rounded-2xl bg-emerald-50 border border-emerald-200 space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-mono font-bold uppercase text-emerald-800">Groundedness Score</span>
        ...
        <div className="p-5 rounded-2xl bg-amber-50 border border-amber-200 space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-mono font-bold uppercase text-amber-800">Source Publications</span>
        ...
        <div className="p-5 rounded-2xl bg-indigo-50 border border-indigo-200 space-y-2">
          <div className="flex items-center justify-between">
            <span className="text-xs font-mono font-bold uppercase text-indigo-800">Clause-Level Citations</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="p-6 bg-white rounded-2xl border border-gray-200 shadow-sm space-y-3">
          <div className="flex items-center gap-2 text-bis-navy font-bold text-base">
            <Layers className="w-5 h-5 text-bis-navy" />
        ...
        <div className="p-6 bg-white rounded-2xl border border-gray-200 shadow-sm space-y-3">
          ...
          <ul className="text-xs text-gray-600 space-y-1.5 list-disc pl-5">
            <li><strong>Automated Execution:</strong> Run via automated CI/CD and verifiable locally with <code className="bg-gray-100 px-1 py-0.5 rounded text-[11px] font-mono">python3 tests/run_benchmark.py</code>.</li>
      ...
      <section className="bg-white rounded-2xl border border-gray-200 shadow-sm p-6 space-y-4">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 border-b border-gray-200 pb-4">
          <div>
            <h2 className="text-lg font-bold text-bis-ink">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="text-xs font-bold text-bis-navy hover:text-bis-red transition-colors flex items-center gap-1"
            >
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 text-xs border border-gray-300 rounded-xl bg-gray-50 text-bis-ink focus:outline-none focus:ring-2 focus:ring-bis-red"
            />
          </div>
        </div>
            ...
            <tbody className="divide-y divide-gray-100 font-sans">
              {filteredCases.slice(0, 20).map((c: any, idx: number) => (
                <tr key={c.id || idx} className="hover:bg-gray-50 transition-colors">
                  <td className="py-2.5 px-3 font-mono font-bold text-gray-500 text-[11px]">
                    {c.id || `TC-${idx + 1}`}
                  </td>
                  <td className="py-2.5 px-3 text-gray-600 text-[11px]">
                    {c.category || (c.is_abstention ? 'Out of Corpus' : 'General')}
                  </td>
                  <td className="py-2.5 px-3 font-medium text-bis-ink">
                    <button
                      onClick={() => {
                        setQueryPrefill(c.query);
                        setActiveTab('chat');
                      }}
                      className="text-left hover:text-bis-red transition-colors cursor-pointer"
                      title="Test this query in Chat Workspace"
                    >
                      {c.query}
                    </button>
                  </td>
                  <td className="py-2.5 px-3 font-mono text-[11px] text-bis-navy font-semibold">
        ...
        {filteredCases.length > 20 && (
          <div className="text-center pt-2">
            <button
              onClick={() => setActiveTab('analytics')}
              className="text-xs font-bold text-bis-navy hover:underline"
            >
