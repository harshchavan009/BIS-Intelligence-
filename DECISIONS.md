...
### Decision 12: Evaluator QA Refinements, Multi-Query Standards Smoke Test & Dynamic Runtime Dates
- **Decision**: Address screenshot QA findings ahead of evaluator and judge review:
  1. **Standards Finder Multi-Query Robustness**: Enhanced structured matching in `retriever.py` with plural/singular stemming, token decompounding (e.g., "smartwatch" -> "smart watch"), industry aliases ("tmt", "lpg", "opc", "ppc", "cooker", "iron"), and normalized IS-number digit matching. Added common consumer appliances (`IS 302` Electric Iron, `IS 2347` Pressure Cooker, `IS 15683` Fire Extinguishers) to `is_product_map.json`. Added permanent multi-query smoke test (`test_02c_standards_multi_query_smoke`) in `test_backend.py`.
  2. **Bulletproof Timeline Step Fallbacks**: Initialized `timelineSteps` directly with verified step definitions on component mount, and guarded JSX rendering so Scheme-IV (and all schemes) can never render a blank list even during transient network latency.
  3. **Zero-Credential Exposure & Template Auth**: Confirmed zero credentials exist in frontend bundles or login form state/placeholders. Documented environment-based evaluator auth variables in `.env.example`.
  4. **Dynamic Build & Evaluation Timestamps**: Injected `__BUILD_DATE__` at build time in Vite, and wired "Last Updated" in `AccessibilityBar.tsx` and "Last Evaluated" in `AboutPage.tsx` to read dynamically from `/api/health` and the evaluation harness artifact (`data/eval_results.json`).
  5. **Defensible Accessibility Claim & Corpus Framing**: Reworded footer badge to "Built to GIGW 3.0 & WCAG 2.1 AA Guidelines". Unified standards count language across Home, About, Registry, and TourGuide as "40+ standards across our 7-document pilot corpus".
- **Rationale**: Eliminates evaluator friction points, prevents query regressions, ensures claim defensibility, and guarantees consistent, fresh metadata across the application.
